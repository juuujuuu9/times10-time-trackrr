import { pgTable, serial, varchar, text, boolean, timestamp, foreignKey, integer, unique, numeric, primaryKey } from "drizzle-orm/pg-core"
...
export const userTaskLists = pgTable("user_task_lists", {
	...
	foreignKey({
			columns: [table.userId],
			foreignColumns: [users.id],
			name: "user_task_lists_user_id_users_id_fk"
		}),
	foreignKey({
			columns: [table.taskId],
			foreignColumns: [tasks.id],
			name: "user_task_lists_task_id_tasks_id_fk"
		}),
]);
...
export const projectTeams = pgTable("project_teams", {
	...
	foreignKey({
			columns: [table.teamId],
			foreignColumns: [teams.id],
			name: "project_teams_team_id_teams_id_fk"
		}),
	foreignKey({
			columns: [table.assignedBy],
			foreignColumns: [users.id],
			name: "project_teams_assigned_by_users_id_fk"
		}),
	primaryKey({ columns: [table.projectId, table.teamId], name: "project_teams_project_id_team_id_pk"}),
]);
