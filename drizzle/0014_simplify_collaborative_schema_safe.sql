-- Safe Simplified Collaborative Schema Migration
-- Preserves existing data while implementing direct relationships
-- Following RULE-001: Simplicity First

-- Step 1: Add team_id to tasks table for direct team association (nullable first)
ALTER TABLE "tasks" ADD COLUMN "team_id" integer;

-- Step 2: Add project_id to teams table for direct project association (nullable first)
ALTER TABLE "teams" ADD COLUMN "project_id" integer;

-- Step 3: Migrate existing data from project_teams to teams.project_id
UPDATE "teams" 
SET "project_id" = (
  SELECT "project_id" 
  FROM "project_teams" 
  WHERE "project_teams"."team_id" = "teams"."id" 
  LIMIT 1
)
WHERE EXISTS (
...
-- Step 4: Migrate existing data from task_collaborations to tasks.team_id
UPDATE "tasks" 
SET "team_id" = (
  SELECT "team_id" 
  FROM "task_collaborations" 
  WHERE "task_collaborations"."task_id" = "tasks"."id" 
  LIMIT 1
)
WHERE EXISTS (
  SELECT 1 FROM "task_collaborations" 
  WHERE "task_collaborations"."task_id" = "tasks"."id"
);
...
-- Step 6: Add foreign key constraints for simplified schema
...
-- Step 8: Update existing foreign key constraints to use CASCADE for better data integrity
...
-- Step 9: Update task-related foreign keys to use CASCADE
...
-- Step 10: Update notifications foreign key
...
-- Step 11: Drop the old junction tables after data migration
