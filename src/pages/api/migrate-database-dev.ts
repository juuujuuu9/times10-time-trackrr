...
export const POST: APIRoute = async (context) => {
  try {
    console.log('Starting database migration (dev access)...');
    ...
    if (checkColumn.rows.length > 0) {
      return new Response(JSON.stringify({ 
        success: true, 
        message: 'project_id column already exists - migration already completed!' 
      }), {
        status: 200,
    ...
    return new Response(JSON.stringify({ 
      success: true, 
      message: 'Migration completed successfully! Database schema updated.' 
    }), {
      status: 200,
      headers: { 'Content-Type': 'application/json' }
    });
